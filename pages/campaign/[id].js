...
export async function getServerSideProps({ params }) {
  ...
  const contract = new ethers.Contract(campaignId,Campaign.abi,provider)
  ...
  return {
    props: {
      id: campaignId,
      minimumContribution: ethers.utils.formatUnits(summary[0], 'ether') ,
      balance: ethers.utils.formatUnits(summary[1], 'ether'),
      requestsCount: parseInt(summary[2]),
      votersCount: parseInt(summary[3]),
      managerAddress: summary[4],
      name: summary[5],
      description: summary[6],
      image: summary[7],
      target: ethers.utils.formatUnits(summary[8], 'ether'),
      managerName: summary[9],
      ETHPrice,
    },
  };
}
...
export default function CampaignSingle({
 ...
 const {currentAccount, currentProvider} = useContext(AccountContext)
  ...
  async function onSubmit(data) {
    if (typeof window.ethereum !== 'undefined') {
      ...
      try {
        if (minimumContribution > ethers.utils.parseEther(data.value)) {
          throw new Error(`Contribution must be higher than minimum contribution`);
        }
        
        const contributed = await contract.contribute({
          value: ethers.utils.parseEther(data.value),
        })
        await currentProvider.waitForTransaction(contributed.hash)  
          ...
          >
            <Stack spacing={{ base: 6 }}>
              ...
              <Link
                ...
                isExternal
              >
                View on Mumbai Polygon Scan <ExternalLinkIcon mx="2px" />
              </Link>
              <Box mx={"auto"} w={"full"}>
                <SimpleGrid columns={{ base: 1 }} spacing={{ base: 5 }}>
                  ...
                  />
                  <StatsCard
                    title={"Name of Campaign Creator"}
                    stat={managerName}
                    info={
                      "The Campaign Creator created the campaign and can create requests to withdraw money."
                    }
